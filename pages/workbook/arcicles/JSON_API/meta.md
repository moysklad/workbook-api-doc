...
Под метаданными объекта (конкретный объект в МоемСкладе, например, товар "Мяч футбольный") понимается краткое представление сущности (товар, услуга, приемка и т.д.), которое содержится в поле `meta`.
