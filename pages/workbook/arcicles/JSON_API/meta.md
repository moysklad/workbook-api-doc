...
Под метаданными объекта понимается краткое представление сущности, которое содержится в поле `meta`.
...
Метаданные сущности описывают перечисляемые поля сущностей: статусы, типы цен, доп.поля и т.д.
...
Изменим товар, указав ему единицу измерения.
```shell
curl -X PUT \
...
Подробнее о ресурсе метаданных сущностей можно узнать в [документации](https://online.moysklad.ru/api/remap/1.1/doc/index.html).