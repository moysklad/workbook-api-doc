...
# Вебхуки
## Что такое вебхук?
Вебхук - это механизм отправки уведомлений при наступлении в системе события, на которое подписано клиентское приложение.  Под событием понимается изменение состояния системы. Например, событиями являются создание нового товара, изменение полей у контрагента, удаление заказа покупателя. Уведомлением будет запрос метода POST, который будет содержать информацию о наступившем событии. Например, при наступлении нового события, будет отправлено уведомление, которое будет содержать ссылку на созданный товар.

В основе механизма лежит реакция на определенное событие, которое инициирует отправку уведомления, что в свою очередь также является событием, но для клиентского приложения. Данный подход соответствует принципам сервис-ориентированной архиектуры (SOA).

Непосредственно сам вебхук содержит описание изменение, которое отправляется на указанный url.
...
Можно привести следующую аналогию. Предположим вы заказали товар, но его не оказалось в наличии, вы каждый день звоните в магазин, чтобы уточнить о появлении товара, это похоже на опрос через АПИ. Но  вы можете просто попросить менеджера в магазине позвонить вам по указанному номеру телефона, когда товар появится, это подписка на вебхуки.
Очевидно, что подписка на вебхуки эффективнее и проще, так как получается значительно меньше нагрузки на клиентское приложение (не нужно опрашивать сервер, обрабатывать ответ), как и на сервер.
...
У вебхука можно изменить поля, указанные при создании, а также включить/отключить вебхук. Для этого выполняется PUT запрос с указанием идентификатора. Пример запроса с изменением события
```shell
curl -X PUT \
  https://online.moysklad.ru/api/remap/1.1/entity/webhook/a5b3cd1f-caee-11e8-9ff4-34e80022dcb3 \
  -H 'Authorization: Basic token==' \
...
* изменение остатков, цен и штрихкодов недоступны для вебхуков
...
2. Получаем уиикальный url. Необходимо скопировать его
...
Мы советуем использовать вебхуки, чтобы сделать взаимодействие МоегоСклада с вашим интернет-магазином или приложением простым и удобным.