...
JSON API позволяет работать со значениями дополнительных полей, обращаясь к конкретному объекту (документу). Это подробно описано в статье ([attributes](https://github.com/moysklad/workbook-api-doc/blob/master/pages/workbook/arcicles/JSON_API/attributes.md)). В данной же статье рассмотрим на примере магазина ноутбуков просмотр, создание, редактирование и удаление самих дополнительных полей средствами JSON API.

Рассмотрим в качестве примера работу магазина по продаже ноутбуков. Предположим, что нам необходимо сортировать ноутбуки по некоторым характеристикам, которых нет в свойствах товара по умолчанию. Например, материал корпуса, наличие CD/DVD-Rom, наличие разъема Type-C и т.д. 
...
Доступно также массовое создание дополнительных полей. Для этого надо передать в теле POST-запроса массив со свойствами каждого из создаваемых атрибутов.
